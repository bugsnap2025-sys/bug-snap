 ...
 * Tries multiple endpoints to ensure validity.
 */
export const validateClickUpToken = async (token: string): Promise<boolean> => {
    try {
        ...
        if (userRes.status === 401) {
            ...
            // If both fail with 401/403, strictly invalid
            if (teamRes.status === 401 || teamRes.status === 403) return false;
        }

        // If explicit 401/403 from primary endpoint and we didn't return above
        ...
        throw new Error("Connection Error: Unable to reach ClickUp. Please check your internet.");
    }
};
